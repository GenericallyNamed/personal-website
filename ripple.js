...
function ripple(elem, rgb) {    
    ...
    var styleLeft = (x - ((rippleSize)));   //This calculates and stores the position values that will be used for the ripple object
    var styleTop = (y - ((rippleSize)));    //This calculates and stores the position values that will be used for the ripple object
    rippleObject.style.position = "absolute";
    ...
    rippleObject.addEventListener("mouseup", function() {
        ...
        if(rippleObject.rippleStatus == "active") {
            ...
            setTimeout(function() {         //This setTimeout ensures that the fade-out animation on the ripple object will be allowed to run before the actual ripple 
                object.parentNode.remove(); //object is deleted after 2 seconds.
                object.remove();
            }, 2000);
        }*/
    });
    ...
    rippleObject.addEventListener("touchend", function() {
        removeRipple(this);
    });
    rippleObject.addEventListener("touchcancel", function() {
        removeRipple(this);
    });
    rippleObject.addEventListener("touchmove", function() {
        removeRipple(this);
    });
    return;
}
...
function removeRipple(rippleObject) {
    if(rippleObject.rippleStatus == "active") {
        rippleObject.rippleStatus = "remove";
        rippleObject.classList.add('rippleEnd');
        setTimeout(function() {         //This setTimeout ensures that the fade-out animation on the ripple object will be allowed to run before the actual ripple 
            rippleObject.parentNode.remove(); //object is deleted after 2 seconds.
            rippleObject.remove();
        }, 2000);
    }
}